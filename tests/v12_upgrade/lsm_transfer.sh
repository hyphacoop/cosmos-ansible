...
tests/v12_upgrade/log_lsm_data.sh happy-transfer pre-transfer-1 $owner_address -
...
tests/v12_upgrade/log_lsm_data.sh happy-transfer post-transfer-1 $owner_address -
owner=$($CHAIN_BINARY q staking tokenize-share-record-by-denom $tokenized_denom --home $HOME_1 -o json | jq -r '.record.owner')
echo "$owner owns record $record_id."
...
tests/v12_upgrade/log_lsm_data.sh happy-transfer pre-transfer-2 $owner -
...
tests/v12_upgrade/log_lsm_data.sh happy-transfer post-transfer-2 $owner -
owner=$($CHAIN_BINARY q staking tokenize-share-record-by-denom $tokenized_denom --home $HOME_1 -o json | jq -r '.record.owner')
echo "$owner owns record $record_id."
