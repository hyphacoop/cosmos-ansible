...
submit_tx "tx ibc-transfer transfer transfer channel-1 $WALLET_5 10000000$tokenized_denom --from $WALLET_3 -o json --gas auto --gas-adjustment 1.2 --fees $BASE_FEES$DENOM -b block -y" $CHAIN_BINARY $HOME_1
...
submit_tx "tx ibc-transfer transfer transfer channel-1 $WALLET_5 10000000$tokenized_denom --from $WALLET_3 -o json --gas auto --gas-adjustment 1.2  --fees $CONSUMER_FEES$DENOM -b block -y" $STRIDE_CHAIN_BINARY $STRIDE_HOME_1
