...
echo "** FAILURE CASES> 2: TOKENIZE BOND DELEGATIONS **"
    ...
    submit_tx "tx staking validator-bond $VALOPER_1 --from $failure_bonding_account -o json --gas auto --gas-adjustment $GAS_ADJUSTMENT -y --fees $BASE_FEES$DENOM" $CHAIN_BINARY $HOME_1
...
echo "** FAILURE CASES> 3: TOKENIZE TO BREACH THE VALIDATOR LIQUID STAKING CAP **"
    ...
    submit_bad_tx "tx staking tokenize-share $VALOPER_2 100000000$DENOM $failure_liquid_account --from $failure_liquid_account -o json --gas auto --gas-adjustment $GAS_ADJUSTMENT --fees $BASE_FEES$DENOM -y" $CHAIN_BINARY $HOME_1
