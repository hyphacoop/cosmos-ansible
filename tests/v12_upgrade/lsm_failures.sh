...
echo "Failure case 5: Attempt to tokenize with WALLET_5 after disabling tokenizing..."
...
# Cleanup
submit_tx "tx staking unbond $VALOPER_1 $wallet_4_delegations --from $WALLET_4 -o json --gas auto --gas-adjustment $GAS_ADJUSTMENT --fees $BASE_FEES$DENOM -y" $CHAIN_BINARY $HOME_1
submit_tx "tx staking unbond $VALOPER_2 $wallet_4_delegations --from $WALLET_4 -o json --gas auto --gas-adjustment $GAS_ADJUSTMENT --fees $BASE_FEES$DENOM -y" $CHAIN_BINARY $HOME_1
submit_tx "tx staking unbond $VALOPER_1 $wallet_5_delegations --from $WALLET_5 -o json --gas auto --gas-adjustment $GAS_ADJUSTMENT --fees $BASE_FEES$DENOM -y" $CHAIN_BINARY $HOME_1
submit_tx "tx staking unbond $VALOPER_2 $wallet_5_delegations --from $WALLET_5 -o json --gas auto --gas-adjustment $GAS_ADJUSTMENT --fees $BASE_FEES$DENOM -y" $CHAIN_BINARY $HOME_1
