...
submit_tx "tx bank send $WALLET_1 $ica_address 1000000000$DENOM --gas auto --gas-adjustment 1.2 --fees $BASE_FEES$DENOM -b sync -y -o json" $CHAIN_BINARY $HOME_1
# $CHAIN_BINARY tx bank send $WALLET_1 $ica_address 1000000000$DENOM --gas auto --gas-adjustment 1.2 --fees $BASE_FEES$DENOM -b sync -y --home $HOME_1 -o json | jq '.'
