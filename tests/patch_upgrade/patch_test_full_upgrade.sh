#!/bin/bash
# Upgrade both validators

echo "Upgrading all validators..."
...
sudo systemctl disable $PROVIDER_SERVICE_1 --now
...
echo "Description=Consumer service"       | sudo tee /etc/systemd/system/$PROVIDER_SERVICE_1 -a
...
echo "ExecStart=$HOME/upgrade-binary start --x-crisis-skip-assert-invariants --home $HOME_1" | sudo tee /etc/systemd/system/$PROVIDER_SERVICE_1 -a
