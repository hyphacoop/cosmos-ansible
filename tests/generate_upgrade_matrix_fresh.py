    ...
    if upgrades:
        for upgrade in upgrades:
            if upgrade not in SKIP_TARGET_VERSIONS:
                ...
                includes.append({'gaia_version': version, 'upgrade_version': upgrade, 'upgrade_mechanism': 'cv_manual'})
                includes.append({'gaia_version': version, 'upgrade_version': upgrade, 'upgrade_mechanism': 'cv_auto'})

    else: # Add main branch build
        includes.append({'gaia_version': version, 'upgrade_version': 'main', 'upgrade_mechanism': 'binary'})
        includes.append({'gaia_version': version, 'upgrade_version': 'main', 'upgrade_mechanism': 'cv_manual'})
        includes.append({'gaia_version': version, 'upgrade_version': 'main', 'upgrade_mechanism': 'auto'})
