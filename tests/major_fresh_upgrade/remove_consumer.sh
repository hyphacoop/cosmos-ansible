...
proposal="$CHAIN_BINARY tx gov submit-proposal consumer-removal proposal-remove-$CONSUMER_CHAIN_ID.json --home $HOME_1 --gas auto --fees 1000$DENOM --from $WALLET_1 --keyring-backend test --chain-id $CHAIN_ID -b block -y -o json"
echo $proposal
...
$CHAIN_BINARY tx gov vote $proposal_id yes --home $HOME_1 --gas auto --fees $BASE_FEES$DENOM --from $WALLET_1  --keyring-backend test --chain-id $CHAIN_ID -b block -y
