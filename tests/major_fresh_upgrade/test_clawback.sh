...
# Get upgrade height
upgrade_height=$(gaiad q upgrade applied v15 -o json | jq -r '.header.height')

let pre_upgrade_height=$upgrade_height-1
let post_upgrade_height=$upgrade_height+1

pre_upgrade_acc_type=$(gaiad q account cosmos145hytrc49m0hn6fphp8d5h4xspwkawcuzmx498 --height $pre_upgrade_height -o json | jq -r '."@type"')
post_upgrade_acc_type=$(gaiad q account cosmos145hytrc49m0hn6fphp8d5h4xspwkawcuzmx498 --height $post_upgrade_height -o json | jq -r '."@type"')
