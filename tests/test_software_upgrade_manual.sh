#!/bin/bash 
# Test a manual gaia software upgrade via governance proposal.
# It assumes gaia is running on the local host.

gaia_host=$1
gaia_port=$2
upgrade_version=$3
upgrade_binary_dir=$4
...
if [ ! -d $upgrade_binary_dir ]
then
    mkdir -p $upgrade_binary_dir
fi
wget -O $upgrade_binary_dir/gaiad "$download_path/gaiad-$upgrade_version-linux-amd64"
chmod +x $upgrade_binary_dir/gaiad
