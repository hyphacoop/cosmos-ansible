...
proposal="$CHAIN_BINARY tx gov submit-proposal param-change tests/major_stateful_upgrade/unbonding-prop.json --gas $GAS --gas-adjustment $GAS_ADJUSTMENT --fees $BASE_FEES$DENOM --from $WALLET_1 --keyring-backend test --home $HOME_1 --chain-id $CHAIN_ID -b block -y -o json"
echo $proposal
gaiadout=$($proposal)
echo "gaiad output:"
echo "$gaiadout"
echo "$gaiadout" > ~/artifact/$CONSUMER_CHAIN_ID-tx-UnbondingTime.txt
